...
def save_prints_to_csv_realtime(print_sets, file_path):
    ...
    with open(file_path, 'a', newline='') as file:
        ...
        # Write each set of print statements as a row
...
#def receive_data():
    # Menerima data dari Arduino melalui koneksi serial
    #data_serial = ser.readline().decode().strip()
    #speed_kmph = float(data_serial)
    #print("Data dari Arduino (Serial):", speed_kmph)

#def receive_data():
    #data = ser.read(4)
    #if len(data) == 4:
      #  speed_kmph = struct.unpack('f', data)[0]  # Mengubah data byte menjadi float
     #   print("Received float value:", speed_kmph)
    #else:
        #print("Failed to receive float value")
#global aktifasiLaneKeeping

print_set1 = ['Hello', 'World', 'This is set 1']
print_set2 = ['Python', 'OpenAI', 'ChatGPT', 'This is set 2']
print_sets = [print_set1, print_set2]
file_path = 'ujicobaambildatakecepatan.csv'

aktifasiLaneKeeping = 0
...
def run(
        weights='/usr/local/lib/python3.9/dist-packages/yolov5/best.pt',
    #    weights='yolov5s.pt',  # model path or triton URL
        ...
        nosave=False,  # do not save images/videosdata.encode()
        classes=None,  # filter by class: --class 0, or --class 0 2 3
        agnostic_nms=False,  # class-agnostic NMS
        augment=False,  # augmented inference
        visualize=False,  # visualize features
        update=False,  # update all models
        project='/home/user/yoloresult',  # save results to project/name
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        cclass = c + 1 
                        global aktifasiLaneKeeping
                        data = f"{cclass},{aktifasiLaneKeeping}"
                        ser.write(data.encode())
                        #data = f"{cclass},{adjspeed},{aktifasiLaneKeeping}"
                        #rpm = ser.read(4)
                        #print("Data dari Arduino (Serial):", rpm)
                        new_print_set = [c, conf]
                        # Add the new print statements to the list of print sets
                        print_sets.append(new_print_set)
                        # Save the print sets to the CSV file in real-time
                        save_prints_to_csv_realtime([new_print_set], file_path)
                        
                        #print(c,conf,cclass,rpm)
                        #LOGGER.info(f"{cclass}{conf}{rpm}")
            ...
            if view_img:
                ...
                #data2 = f"{000},{adjspeed},{aktifasiLaneKeeping}"
